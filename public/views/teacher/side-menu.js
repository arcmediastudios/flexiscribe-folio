  ...
  function loadCoursesInSideMenu() {
    ...
    fetch('https://folio.flexiscribe.net/api/user/profile', {
      ...
      .then(data => {
        ...
        if (data.sideMenuCourses && data.sideMenuCourses.length) {
          data.sideMenuCourses.forEach(course => {
            const courseLink = document.createElement('a');
            courseLink.href = `/views/teacher/course-page-teacher.html?courseId=${course._id}`;
            courseLink.textContent = course.name;
            courseLink.className = 'course-link';
            coursesSidebar.appendChild(courseLink);
          });
        } else {
          console.log('No courses found or not added to the side menu.');
        }
      })
      .catch(error => {
